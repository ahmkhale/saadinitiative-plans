...
function loadFont(style) {
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(
      `Required local font is missing: ${filePath}. Install IBM Plex Sans Arabic locally or set SAAD_FONT_DIR.`,
    );
  }
...
export function fitMeasuredText(value, {
  ...
  if (!text || measureText(text, baseSize, style) <= maxWidth) {
    return Object.freeze({
      size: baseSize,
      width: measureText(text, baseSize, style),
      overflow: false,
      reduced: false,
    });
  }
